...
msoa = pd.DataFrame(index=data[2017].index)
for year in years:
    msoa[str(year)] = pd.DataFrame(data[year].drop('population', axis=1).sum(1))

msoa2 = pd.DataFrame(index=data[2007].index)
for year in years:
    msoa2[str(year)] = pd.DataFrame(data[year].drop('population', axis=1).sum(1))
    
only_2011 = msoa.loc[msoa['2007'].isna() == True].index.tolist()
only_2001 = msoa.loc[msoa['2017'].isna() == True].index.tolist()
